...
class GANTrainer:
    ...
    def train_step(self, batch):
        """
        Single GAN training step with AMP and gradient accumulation.
        
        - Generator (Conformer) runs in FP32 for numerical stability
        - Discriminator runs in AMP (FP16) for speed
        - Gradients accumulate over accum_steps before optimizer step
        """
        audio, lengths = batch
        ...
        # Reconstruction loss in FP32 (spectral operations need precision)
        with torch.amp.autocast('cuda', enabled=False):
