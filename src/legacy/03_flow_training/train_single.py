...
PROJECT_ROOT = SCRIPT_DIR.parent.parent  # scripts/03_flow_training -> scripts -> GS-TS
...
class FlowTrainer:
    ...
    def train_step(self, batch: Dict) -> Dict:
        ...
        durations = batch['durations'].to(self.device)  # [B]
        ...
        # Text encoder returns (embeddings, pooled, log_duration)
        ...
        # Flow matching loss
        cfm_loss, info = self.cfm.compute_loss(model_fn, atoms_norm, mask=None)
        
        # Duration prediction loss (MSE on log scale)
        log_duration_target = torch.log(durations.clamp(min=0.1)).unsqueeze(-1)  # [B, 1]
        duration_loss = torch.nn.functional.mse_loss(log_duration_pred, log_duration_target)
        
        # Total loss
        duration_weight = self.config['training'].get('duration_loss_weight', 0.1)
        loss = cfm_loss + duration_weight * duration_loss
        ...
        info['duration_loss'] = duration_loss.item()
    ...
    def validate(self) -> float:
        ...
        for batch in self.val_loader:
            ...
            text_emb, _, _ = self.text_encoder(input_ids, attention_mask)
