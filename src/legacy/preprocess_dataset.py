...
def load_config_from_yaml(config_path: str) -> Dict:
    ...
    config = {
        ...
        "densify_from_iter": yaml_config.get("density_control", {}).get("densify_from_iter", 500),
        "densify_until_iter": yaml_config.get("density_control", {}).get("densify_until_iter", 4000),
...
def fit_atoms_to_audio(
    ...
    # Initialize model (must be per-file as parameters change)
    model = AudioGSModel(
    ...
    # NOTE: renderer and loss_fn are now passed in (reused across files)
    
    # Optimizer - use helper that handles different LRs per param group
    optimizer_config = {
        "lr_amplitude": config["lr_amplitude"],
        "lr_position": config["lr_position"],
        "lr_frequency": config["lr_frequency"],
        "lr_sigma": config["lr_sigma"],
        "lr_phase": config["lr_phase"],
        "lr_chirp": config["lr_chirp"],
    }
    optimizer = rebuild_optimizer_from_model(model, torch.optim.Adam, optimizer_config)

    
    # Learning rate scheduler
    scheduler = torch.optim.lr_scheduler.MultiStepLR(
        optimizer, 
        milestones=[2000, 3000, 3500], 
        gamma=0.5
    )
    ...
    # Training loop (stripped down for speed)
    ...
    for iteration in range(config["num_iters"]):
        ...
        if config["densify_from_iter"] <= iteration < config["densify_until_iter"]:
            if iteration % config["densification_interval"] == 0 and iteration > 0:
