...
class GaborGridEncoder(nn.Module):
    ...
    def _init_custom_bias(self):
        ...
        if final_layer.bias is not None:
            with torch.no_grad():
                ...
                # Add random noise to break symmetry and prevent constant output
                bias[:, :, 0].fill_(-2.0)
                bias[:, :, 0] += torch.randn_like(bias[:, :, 0]) * 0.5
                ...
                # 5. Sigma: 2.5 -> softplus(0.5)*0.02 + 0.002 ≈ 0.012s (12ms)
                bias[:, :, 5].fill_(2.5)
    ...
    def forward(
        ...
        # Apply activations
        # Amplitude: softplus with threshold to prevent exp() overflow in mixed precision
        amplitude = F.softplus(amplitude_raw, threshold=20.0).clamp(max=10.0)
        ...
        # Local offsets: Allow movement within ±2 cells (Relaxed constraint)
        ...
        # Sigma: softplus with threshold for stability (prevents exp() overflow)
        # softplus(2.5 - 2.0) * 0.02 + 0.002 = 0.9*0.02 + 0.002 = 0.02s = 20ms (max)
        # softplus(-2.0) * 0.02 + 0.002 ≈ 0.004s = 4ms (min)
        sigma = F.softplus(sigma_raw - 2.0, threshold=20.0) * 0.02 + self.sigma_min
        ...
        # Raw cos/sin for phase regularization (prevent vector collapse → atan2 gradient explosion)
        cos_phi_raw_flat = cos_phi_raw.view(batch_size, n_atoms)
        sin_phi_raw_flat = sin_phi_raw.view(batch_size, n_atoms)
        ...
        result = {
            ...
            'gamma': gamma_flat,
            # For Flow Matching (normalized to unit circle)
            'cos_phi': cos_phi_flat,
            'sin_phi': sin_phi_flat,
            # Raw phase logits for circular regularization loss
            'cos_phi_raw': cos_phi_raw_flat,
            'sin_phi_raw': sin_phi_raw_flat,
            # Metadata
